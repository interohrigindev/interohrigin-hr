-- =====================================================================
-- InterOhrigin HR — 역할 구조 재설계 마이그레이션
-- 6역할 → 4역할 (employee, leader, director, ceo)
-- 8단계 평가 → 6단계 (pending → self_done → leader_done → director_done → ceo_done → completed)
-- Supabase SQL Editor에서 bootstrap-admin.sql 실행 후 이 파일을 실행하세요.
-- =====================================================================

BEGIN;

-- =====================================================================
-- PHASE 1: CHECK 제약조건 변경
-- =====================================================================
...
-- 1-3. evaluator_scores.evaluator_role
...
-- 1-4. evaluation_weights.evaluator_role
...
-- 3-1. generate_evaluation_sheets: employee/leader만 평가 대상
...
BEGIN
  ...
  -- employee/leader 역할만 평가 대상 (임원인 director/ceo는 제외)
  FOR v_employee_id IN
    SELECT id FROM employees
    WHERE is_active = true
      AND role IN ('employee', 'leader')
  LOOP
...
-- 3-2. advance_evaluation_stage: 새 4단계 흐름 + 리더 자동 스킵
...
BEGIN
  ...
  -- (리더는 자기 평가를 자기가 하지 않으므로 leader_done 단계를 건너뜀)
...
-- 3-3. calculate_final_score: self/leader/director/ceo 4가중치
...
BEGIN
  ...
  IF v_director_total IS NOT NULL THEN
    SELECT weight INTO v_w FROM evaluation_weights
    WHERE period_id = v_period_id AND evaluator_role = 'director';
    IF v_w IS NOT NULL THEN
      v_weighted_sum := v_weighted_sum + v_director_total * v_w;
      v_weight_sum   := v_weight_sum   + v_w;
    END IF;
  END IF;
...
-- 4-1. v_evaluation_summary: leader/director/ceo 3컬럼
...
WITH
  ...
  weighted AS (
    ...
    LEFT JOIN public.evaluation_weights w_dir
      ON w_dir.period_id    = t.period_id AND w_dir.evaluator_role    = 'director'
...
-- 4-2. v_item_scores_comparison: leader/director/ceo 3평가자
...
-- 4-3. v_evaluation_progress: 6단계 카운트
...
CREATE POLICY "target_select_director_up"
  ...
  TO authenticated
  USING (
    public.get_my_role() IN ('director', 'ceo')
  );
...
CREATE POLICY "self_eval_select_evaluator"
  ...
  TO authenticated
  USING (
    public.get_my_role() IN ('leader', 'director', 'ceo')
    AND EXISTS (
      SELECT 1 FROM public.evaluation_targets t
      WHERE t.id = self_evaluations.target_id
        AND (
    ...
    AND EXISTS (
      ...
      WHERE t.id = evaluator_scores.target_id
        AND (
          ...
          OR (evaluator_scores.evaluator_role = 'director' AND t.status = 'leader_done')
          OR (evaluator_scores.evaluator_role = 'ceo'      AND t.status = 'director_done')
        )
    )
  );
    ...
    AND EXISTS (
      ...
      WHERE t.id = evaluator_comments.target_id
        AND (
          ...
          OR (evaluator_comments.evaluator_role = 'director' AND t.status = 'leader_done')
          OR (evaluator_comments.evaluator_role = 'ceo'      AND t.status = 'director_done')
        )
    )
  );
...
-- =====================================================================
-- PHASE 6: grade_criteria RLS 버그 수정
-- =====================================================================
DROP POLICY IF EXISTS "grade_criteria_manage_management" ON public.grade_criteria;

CREATE POLICY "grade_criteria_manage_admin"
  ON public.grade_criteria FOR ALL
  TO authenticated
  USING (public.is_admin());

-- =====================================================================
-- PHASE 7: create_employee_with_auth() RPC 함수
-- =====================================================================
...
-- =====================================================================
-- PHASE 8: 기본 가중치 재설정
-- 기존 가중치 데이터 삭제 (bootstrap에서 이미 삭제됨)
-- 새 평가 기간 생성 시 기본값으로 사용할 참고 정보:
-- self 10%, leader 25%, director 35%, ceo 30%
-- =====================================================================
