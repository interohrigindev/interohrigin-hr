-- 071_fix_health_check_is_active.sql
-- 목적: 헬스체크 함수에서 "column 'is_active' does not exist" 에러 수정
--   evaluation_periods 테이블은 is_active 컬럼이 없고 status='in_progress' 가 활성 표시
--   migration 063 이 미적용된 환경을 위한 방어적 재적용

-- 함수 강제 삭제 후 재생성 (CREATE OR REPLACE 만으로는 매개변수/리턴 타입 충돌 시 실패)
DROP FUNCTION IF EXISTS public.run_all_health_checks();

CREATE FUNCTION public.run_all_health_checks()
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_orphans
  FROM candidates c
  WHERE c.job_posting_id IS NULL
    AND c.status NOT IN ('rejected','hired','no_show');
  IF v_orphans > 0 THEN
    PERFORM record_health_check('orphan_candidates', 'warning',
      EXTRACT(MILLISECOND FROM (clock_timestamp() - v_start))::int,
      jsonb_build_object('count', v_orphans),
      v_orphans || '건의 진행 중 지원자가 공고에 연결되지 않았습니다');
    PERFORM queue_maintenance_task(
      'orphan_candidates', '공고 미연결 진행 중 지원자',
      v_orphans || '건의 지원자가 job_posting_id 없이 진행 중입니다.',
      'candidates', NULL,
      '아래 SQL 로 확인하고 수동으로 공고 매칭 또는 종료 처리.',
      'SELECT id, name, email, status FROM candidates WHERE job_posting_id IS NULL AND status NOT IN (''rejected'',''hired'',''no_show'');',
      'medium'
    );
    v_issues := v_issues + 1;
  ELSE
  ...
  -- ─── 4. migration 060 적용 여부 (portfolio_files 컬럼) ──
  v_start := clock_timestamp();
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'candidates' AND column_name = 'portfolio_files'
  ) THEN
  ...
  -- ─── 5. 활성 직원 이메일 미설정 (employees.is_active 존재 시에만) ─────
  v_start := clock_timestamp();
  ...
  ELSE
    SELECT COUNT(*) INTO v_count FROM employees WHERE role = 'hr_admin';
  END IF;
  IF v_count = 0 THEN
