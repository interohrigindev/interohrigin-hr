-- 096: get_shared_candidate — 로그인 + 권한 (임원/대표) 필수
-- 변경: anon 권한 회수, 인증된 사용자 중 관리자급/임원/대표만 조회 가능
-- 패턴: 수습평가 공유 (probation-share) 와 동일
...
BEGIN
  ...
  -- 2) 권한 체크 — 임원/대표/관리자급만 (수습평가 공유와 동일 정책)
