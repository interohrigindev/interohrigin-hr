...
async function registerKoreanFonts(doc: jsPDF) {
  try {
    ...
    if (!fontCacheRegular.byteLength || !fontCacheBold.byteLength) {
    ...
    console.error('한글 폰트 로딩 실패:', e)
    fontCacheRegular = null
    fontCacheBold = null
    alert('PDF 한글 폰트를 불러오지 못했습니다. 페이지를 새로고침 후 다시 시도해주세요.')
    return false
  }
}
