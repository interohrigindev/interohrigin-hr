...
export default function RecruitmentSettings() {
  ...
  function getPreviewHtml() {
    ...
    // 실제 발송 이메일과 동일한 디자인
    return `
<div style="margin:0;padding:20px 0;background:#f3f4f6;font-family:'Apple SD Gothic Neo','Malgun Gothic',sans-serif;">
  <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,0.05);">
    ...
    <!-- Body -->
    <div style="padding:32px 28px;font-size:14px;color:#374151;line-height:1.7;">
      ${preview
        .replace(/<a /g, '<a style="color:#6B3FA0;font-weight:bold;text-decoration:underline;" ')
        .replace(/<table/g, '<table style="width:100%;border-collapse:collapse;margin:16px 0;background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden"')
        .replace(/<td><strong>/g, '<td style="padding:8px 16px;font-weight:bold;width:100px;vertical-align:top;color:#6B3FA0;border-bottom:1px solid #e5e7eb;"><strong>')
        .replace(/<td>(?!<strong>)/g, '<td style="padding:8px 16px;border-bottom:1px solid #e5e7eb;">')
      }
    </div>
    ...
    <div style="background:#f9fafb;padding:20px 28px;border-top:1px solid #e5e7eb;">
      ...
      <p style="font-size:11px;color:#d1d5db;text-align:center;margin:8px 0 0;">
        (주)인터오리진 | 서울 강남구 선릉로121길 5, 3층(논현동, 인터오리진타워)
      </p>
    </div>
  </div>
</div>`
