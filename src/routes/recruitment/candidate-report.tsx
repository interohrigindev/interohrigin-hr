...
import { ArrowLeft, FileText, Sparkles, Loader2, CheckCircle, XCircle, AlertTriangle, Mail, Copy } from 'lucide-react'
...
다음 JSON 형식으로만 응답하세요 (다른 텍스트 없이):
{
  ...
  function openSurveyEmail() {
    if (!candidate) return
    const surveyUrl = `${window.location.origin}/survey/${candidate.invite_token}`
    const subject = encodeURIComponent(`[인터오리진] ${candidate.name}님, 사전 질의서를 작성해주세요`)
    const body = encodeURIComponent(
      `안녕하세요, ${candidate.name}님.\n\n(주)인터오리진에 지원해주셔서 감사합니다.\n서류 검토가 완료되어 사전 질의서 작성을 요청드립니다.\n\n아래 링크를 클릭하여 질의서를 작성해주세요:\n${surveyUrl}\n\n감사합니다.\n인터오리진 채용팀`
    )
    const a = document.createElement('a')
    a.href = `mailto:${candidate.email}?subject=${subject}&body=${body}`
    a.click()
  }

  function handleCopySurveyLink() {
    if (!candidate) return
    const url = `${window.location.origin}/survey/${candidate.invite_token}`
    navigator.clipboard.writeText(url)
    toast('질의서 링크가 클립보드에 복사되었습니다.', 'info')
  }
  ...
  async function handleResendSurvey() {
    ...
    try {
      openSurveyEmail()
      const resendCount = ((candidate.metadata as any)?.survey_resend_count || 0) + 1
      await supabase
        .from('candidates')
        .update({
          metadata: {
            ...(candidate.metadata || {}),
            survey_resend_count: resendCount,
            survey_last_resent_at: new Date().toISOString(),
          }
        })
        .eq('id', id)
      ...
      toast('이메일 클라이언트가 열렸습니다. 재발송 이력이 기록되었습니다.', 'success')
    } catch (err: any) {
      toast('재발송 실패: ' + err.message, 'error')
    }
    setResending(false)
  }
  ...
  async function handleDecision(decision: 'proceed' | 'reject') {
    ...
    if (error) {
      ...
      if (decision === 'proceed') {
        openSurveyEmail()
        toast('사전 질의서 발송 처리되었습니다. 이메일을 확인하세요.', 'success')
      } else {
              ...
              {candidate.status === 'resume_reviewed' && analysis ? (
                ...
                <>
                  <div className="bg-indigo-50 rounded-lg p-3">
                    <p className="text-xs font-medium text-indigo-600 mb-1">질의서 발송됨</p>
                    <p className="text-xs text-gray-500">지원자가 아직 질의서를 완료하지 않았습니다.</p>
                    {(candidate.metadata as any)?.survey_resend_count > 0 && (
                      <p className="text-xs text-gray-400 mt-1">
                        재발송 {(candidate.metadata as any).survey_resend_count}회
                        {(candidate.metadata as any).survey_last_resent_at &&
                          ` · ${formatDate((candidate.metadata as any).survey_last_resent_at, 'MM/dd HH:mm')}`}
                      </p>
                    )}
                  </div>
                  <Button variant="outline" className="w-full" onClick={handleResendSurvey} disabled={resending}>
                    {resending ? (
                      <><Loader2 className="h-4 w-4 mr-1 animate-spin" /> 발송 중...</>
                    ) : (
                      <><Mail className="h-4 w-4 mr-1" /> 사전 질의서 재발송</>
                    )}
                  </Button>
                  <Button variant="ghost" className="w-full" onClick={handleCopySurveyLink}>
                    <Copy className="h-4 w-4 mr-1" /> 질의서 링크 복사
                  </Button>
                </>
