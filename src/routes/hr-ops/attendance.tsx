...
const STATUS_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  holiday: { bg: 'bg-blue-100', text: 'text-blue-700', dot: 'bg-blue-500' },
}
...
export default function AttendanceManagementPage() {
  ...
  // ─── Data fetch ──────────────────────────────────────
  useEffect(() => { fetchData() }, [currentYear, currentMonth, profile?.id, isAdmin])
  ...
  const myPresentDays = myRecordsThisMonth.filter((r) => r.status === 'normal' || r.status === 'late' || r.status === 'early_leave').length
  const myLateDays = myRecordsThisMonth.filter((r) => r.status === 'late').length
  const myTotalHours = myRecordsThisMonth.reduce((s, r) => s + (r.work_hours || 0), 0)
  ...
  async function handleQuickCheckIn() {
    ...
    if (existing?.check_in) { toast('이미 출근 기록이 있습니다', 'error'); setCheckingIn(false); return }

    if (existing) {
      await supabase.from('attendance_records').update({ check_in: now, status: isLate ? 'late' : 'normal' }).eq('id', existing.id)
    } else {
      await supabase.from('attendance_records').insert({ employee_id: profile.id, date: todayStr, check_in: now, status: isLate ? 'late' : 'normal' })
    }
    setCheckingIn(false)
  ...
  async function handleQuickCheckOut() {
    ...
    const now = new Date().toISOString()
    const existing = records.find((r) => r.employee_id === profile.id && r.date === todayStr)
    if (!existing?.check_in) { toast('출근 기록이 없습니다', 'error'); setCheckingOut(false); return }
    if (existing.check_out) { toast('이미 퇴근 기록이 있습니다', 'error'); setCheckingOut(false); return }
    ...
    await supabase.from('attendance_records').update({
      check_out: now, work_hours: diff, overtime_hours: ot,
      status: diff < 6 ? 'early_leave' : existing.status,
    }).eq('id', existing.id)
    setCheckingOut(false)
    toast(`퇴근 완료 (${formatTime(now)}) — ${diff}시간 근무`, 'success')
    fetchData()
  }
  ...
  async function handleAdminCheckSubmit() {
    ...
    if (adminCheckType === 'in') {
      ...
      const h = new Date(now).getHours(); const m = new Date(now).getMinutes()
      const isLate = h > 9 || (h === 9 && m > 0)
      if (existing) {
        ...
        await supabase.from('attendance_records').insert({ employee_id: adminEmployeeId, date: targetDate, check_in: now, status: isLate ? 'late' : 'normal', note: adminNote || null })
      }
    } else {
          ...
          <div className="grid grid-cols-7">
            ...
            {calendarDays.map((dayData) => {
              ...
              const hasAbsent = isAdmin
                ? employees.length > dayRecords.length && !isWeekend
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-0.5">
                    ...
                    {hasHoliday && <div className="w-2 h-2 rounded-full bg-blue-500" title="휴가" />}
                      ...
                      {(isAdmin ? dayRecords.slice(0, 2) : dayRecords).map((r) => {
                        ...
                        return (
                          <div key={r.id} className={`flex items-center gap-1 px-1 py-0.5 rounded text-[9px] ${sc.bg} ${sc.text}`}>
                            ...
                            {r.check_out && <span>~{formatTime(r.check_out)}</span>}
                          </div>
                        )
                      })}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50/80">
                      ...
                      <th className="text-center py-2.5 px-3 font-medium text-gray-500 text-xs">근무</th>
                    ...
                    {selectedDayRecords.map((r) => {
                      ...
                      return (
                        <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                          ...
                          <td className="py-2 px-3 text-center font-medium text-gray-900">{formatTime(r.check_in) || '-'}</td>
                          <td className="py-2 px-3 text-center font-medium text-gray-900">{formatTime(r.check_out) || '-'}</td>
                          <td className="py-2 px-3 text-center text-gray-600">{r.work_hours != null ? `${r.work_hours}h` : '-'}</td>
                          <td className="py-2 px-3 text-center">
                            {r.overtime_hours != null && r.overtime_hours > 0
                              ? <span className="text-orange-600 font-medium">{r.overtime_hours}h</span>
